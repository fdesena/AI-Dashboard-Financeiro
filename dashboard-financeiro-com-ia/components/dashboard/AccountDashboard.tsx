...
import { Transaction, RawTransaction, Summary, CategoryData, TimeFilter, TimeFilterType, ComparisonCategoryData, CategoryGoalData, GoalComparisonData, AverageSpendData, AnalysisData } from '../../types';
import { categorizeTransactions, ACCOUNT_CATEGORIES, generateAccountAnalysis } from '../../services/geminiService';
import { formatCurrency, parseCurrencyValue, parseDate, formatDate, getPreviousPeriod } from '../../utils/helpers';
import { ArrowUpCircleIcon, ArrowDownCircleIcon, BanknoteIcon, RotateCwIcon, DownloadIcon, FileSpreadsheetIcon, FileTextIcon, TrashIcon, CheckIcon, XIcon } from '../icons';
...
export const AccountDashboard: FC<AccountDashboardProps> = ({ transactions, setTransactions, processedFiles, setProcessedFiles }) => {
    const [isCategorizing, setIsCategorizing] = useState(false);
    const [timeFilter, setTimeFilter] = useState<TimeFilter>({ type: 'all' });
    const [goals, setGoals] = useState<Record<string, number>>({});
    const [accountCategories, setAccountCategories] = useState<string[]>(() => [...ACCOUNT_CATEGORIES].sort());
    ...
    const handleSaveNewCategory = (transactionId: string) => {
        ...
        if (!accountCategories.includes(trimmedCategory)) {
            setAccountCategories(prev => [...prev, trimmedCategory].sort());
        }
    ...
    const handleUpdateTransactionCategory = (transactionId: string, newCategoryValue: string) => {
    ...
    const handleFiles = async (data: RawTransaction[]) => {
        setIsCategorizing(true);
        const uniqueData = data.filter(row => {
            const rowSignature = `${row.Data}-${row.Descrição}-${row.Valor}`;
            return !processedFiles.includes(rowSignature);
        });
        if (uniqueData.length === 0) { setIsCategorizing(false); return; }
        const newSignatures = uniqueData.map(row => `${row.Data}-${row.Descrição}-${row.Valor}`);
        const descriptions = uniqueData.map(d => d.Descrição);
        const categories = await categorizeTransactions(descriptions);
        const newTransactions: Transaction[] = uniqueData.map((row, index) => ({
            id: `${Date.now()}-${index}`,
            date: parseDate(row.Data)?.toISOString() ?? new Date().toISOString(),
            description: row.Descrição,
            amount: parseCurrencyValue(row.Valor),
    ...
    const processAccountFiles = (files: File[]) => {
        let allData: RawTransaction[] = [];
        let filesRemaining = files.length;
        const headerMap: {[key: string]: string} = {
            'data': 'Data', 'descrição': 'Descrição', 'descricão': 'Descrição', 'descricao': 'Descrição', 'description': 'Descrição', 'histórico': 'Descrição',
        ...
        files.forEach(file => {
            Papa.parse<RawTransaction>(file, {
                ...
                complete: (results) => {
                    if (!results.data.every(row => row.Data && row.Descrição && row.Valor)) {
                        alert(`Arquivo '${file.name}' não contém as colunas necessárias ('Data', 'Descrição', 'Valor') ou elas estão com nomes diferentes. Verifique o arquivo e tente novamente.`);
                        filesRemaining--;
                        if (filesRemaining === 0 && allData.length > 0) handleFiles(allData);
                        else if (filesRemaining === 0) setIsCategorizing(false);
                        return;
                    }
                    const validData = results.data.filter(row => row.Data && row.Descrição && row.Valor);
    ...
    const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.currentTarget.files && e.currentTarget.files.length > 0) {
            processAccountFiles(Array.from(e.currentTarget.files));
        }
    ...
    const handleExportExcel = async () => {
        ...
        const consolidatedData = [
            { Item: 'Resumo - Receitas', Valor: summaryData.income },
            { Item: 'Resumo - Despesas', Valor: summaryData.expenses },
            { Item: 'Resumo - Saldo', Valor: summaryData.balance },
            {},
            { Item: 'Metas por Categoria de Despesa' },
    ...
    const handleExportPdf = async () => {
        ...
        if (dashboardRef.current) {
            await exportToPdf(dashboardRef.current, 'Relatorio_Conta');
        }
        setIsExporting(false);
    };
    ...
    const incomeCategoryData = useMemo<CategoryData[]>(() => {
    ...
    const averageSpendData = useMemo<AverageSpendData[]>(() => {
        ...
        const categoryStats = expenses.reduce((acc, t) => {
            ...
            acc[t.category].total += Math.abs(t.amount); acc[t.category].count += 1;
            return acc;
    ...
    const averageIncomeData = useMemo<AverageSpendData[]>(() => {
        const income = filteredTransactions.filter(t => t.amount > 0);
        if (income.length === 0) return [];
        const categoryStats = income.reduce((acc, t) => {
            if (!acc[t.category]) { acc[t.category] = { total: 0, count: 0 }; }
    ...
    const categoryGoalsData: CategoryGoalData[] = useMemo(() => {
        return expenseCategoryData.map(category => ({
            name: category.name,
    ...
    const comparisonData = useMemo<ComparisonCategoryData[]>(() => {
        ...
        const currentIncome = filteredTransactions.filter(t => t.amount > 0).reduce((sum, t) => sum + t.amount, 0);
        const previousIncome = previousPeriodTransactions.filter(t => t.amount > 0).reduce((sum, t) => sum + t.amount, 0);
        const incomeComparison: ComparisonCategoryData = { name: 'Receitas', current: currentIncome, previous: previousIncome };
        const currentExpenses = filteredTransactions.filter(t => t.amount < 0);
        const previousExpenses = previousPeriodTransactions.filter(t => t.amount < 0);
        const currentExpenseData = currentExpenses.reduce((acc, t) => {
            acc[t.category] = (acc[t.category] || 0) + Math.abs(t.amount); return acc;
        }, {} as Record<string, number>);
        const previousExpenseData = previousExpenses.reduce((acc, t) => {
            acc[t.category] = (acc[t.category] || 0) + Math.abs(t.amount); return acc;
        }, {} as Record<string, number>);
        const allCategories = [...new Set([...Object.keys(currentExpenseData), ...Object.keys(previousExpenseData)])];
        const expenseComparison = allCategories
            .map(category => ({ name: category, current: currentExpenseData[category] || 0, previous: previousExpenseData[category] || 0 }))
    ...
    const handleGenerateAnalysis = async () => {
        ...
        const result = await generateAccountAnalysis(analysisData);
        setAnalysis(result);
        ...
        <div className="p-4 md:p-6">
            <div className="flex justify-end gap-2 mb-4">
                 <button onClick={() => fileInputRef.current?.click()} className="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded-lg no-print">
                    {transactions.length > 0 ? 'Adicionar Mais Arquivos' : 'Adicionar Arquivos (.csv)'}
                </button>
            ...
            <div ref={dashboardRef}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <SummaryCard title="Receitas" value={summaryData.income} icon={<ArrowUpCircleIcon className="w-8 h-8 text-green-500" />} />
                    <SummaryCard title="Despesas" value={summaryData.expenses} icon={<ArrowDownCircleIcon className="w-8 h-8 text-red-500" />} />
                    <SummaryCard title="Saldo" value={summaryData.balance} icon={<BanknoteIcon className="w-8 h-8 text-sky-500" />} />
                </div>
                
                {incomeCategoryData.length > 0 && (
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        <CategoryChart data={incomeCategoryData} title="Receitas por Categoria" />
                        <AverageSpendChart data={averageIncomeData} title="Receita Média por Transação" barColor="#22c55e" barName="Receita Média" totalLabel="Total Recebido" />
                    </div>
                )}
    
                {expenseCategoryData.length > 0 && (
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        <CategoryChart data={expenseCategoryData} title="Despesas por Categoria" />
                ...
                <div className="mt-6 bg-gray-800 p-6 rounded-lg shadow-lg">
                    ...
                    <div className="overflow-y-auto max-h-96">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {transactions.length === 0 ? (
                                    <tr><td colSpan={5} className="text-center py-20 text-gray-500">Clique em "Adicionar Arquivos" para ver suas transações.</td></tr>
                                        ...
                                        <tr key={t.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                                            ...
                                            <td className="px-6 py-4">
                                                {editingRow && editingRow.id === t.id ? (
                                                    ...
                                                    >
                                                        {accountCategories.map(cat => ( <option key={cat} value={cat}>{cat}</option>))}
                                                        <option value="--ADD_NEW--">-- Adicionar Nova --</option>
                                                    </select>
                                                )}
                                            </td>
                                            <td className={`px-6 py-4 text-right font-semibold ${t.amount > 0 ? 'text-green-400' : 'text-red-400'}`}>{formatCurrency(t.amount)}</td>
